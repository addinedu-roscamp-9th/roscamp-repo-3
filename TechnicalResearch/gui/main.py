...
from PyQt5.QtWidgets import (
    ...
    QInputDialog, QListWidget, QGroupBox, QDialog, QListWidgetItem
)
...
class PinkyRobotGUI(QWidget):
    ...
    def create_home_screen(self):
        ...
        title.setStyleSheet("font-size: 48px; font-weight: bold; color: white; margin-bottom: 20px;")
        ...
        btn_nav = self.create_menu_button("Pinky", "#2196F3", lambda: self.stack.setCurrentIndex(1))
        button_layout.addWidget(btn_nav)
        
        btn_schedule = self.create_menu_button("Schedule", "#4CAF50", lambda: self.stack.setCurrentIndex(2))
    ...
    def create_schedule_screen(self):
        ...
        btn_run.setStyleSheet("background: #2ecc71; color: white; font-weight: bold; height: 40px;")
        btn_run.clicked.connect(self.run_selected_preset)
        preset_btn_layout.addWidget(btn_run)
        
        btn_add = QPushButton("➕ 추가")
        btn_add.setStyleSheet("background: #3498db; color: white; font-weight: bold; height: 40px;")
        btn_add.clicked.connect(self.add_new_schedule)
        preset_btn_layout.addWidget(btn_add)
        
        btn_delete = QPushButton("🗑️ 삭제")
        btn_delete.setStyleSheet("background: #e74c3c; color: white; font-weight: bold; height: 40px;")
    ...
    def handle_bring_item(self):
        """물건 가져오기"""
        items = ["💊 약", "🍫 초코과자", "🍘 쌀과자"]
        item, ok1 = QInputDialog.getItem(self, "물건 선택", "가져올 물건:", items, 0, False)
        
        if ok1 and item:
            room, ok2 = QInputDialog.getItem(self, "방 선택", "가져다 줄 방:", self.preset_locations, 0, False)
            ...
            if ok2 and room:
                try:
                    ...
                    if response.status_code == 200:
                        QMessageBox.information(self, "작업 시작", f"{item}를 {room}으로 가져옵니다!")
                except Exception as e:
                    QMessageBox.critical(self, "오류", f"서버 연결 실패: {str(e)}")
    
    def handle_put_item(self):
        """물건 갖다놓기"""
        room, ok = QInputDialog.getItem(self, "현재 위치", "현재 방:", self.preset_locations, 0, False)
    ...
    def add_new_schedule(self):
        """새 스케줄 추가"""
        name, ok = QInputDialog.getText(self, "스케줄 이름", "스케줄 이름을 입력하세요:")
